...
**KVS adoption of sidecar-less files (`open`'s `adopt`)**: A value file can be on disk with no metadata sidecar — written into the store directory out-of-band, by a CLI or anything else that isn't this library — so no key addresses it. `open`/`open_with_clock` take `adopt: &[AdoptionCandidate]`, and right after the store `__metadata__.json` step each candidate gets the sidecar it is missing, making the file an ordinary record that `get_value`, `list_keys` and `get_public_url` all see — which is what the bare-file probe never managed. Value bytes are never touched, and neither is the store's own metadata (no `accessedAt`/`modifiedAt` bump). The core infers nothing from an extension: the caller's declarations are the only source of content types. I/O errors propagate and fail the open.

`AdoptionCandidate` is an enum of two kinds. `Key { key, files: Vec<AdoptableFile { filename, content_type }> }` binds one declared key to at most one of its declared filenames (each of which goes through `validate_filename`; metadata filenames are skipped, since a sidecar is never an adoptable value) — the canonical case being a run's input (`INPUT`, `INPUT.json`, ...), though nothing is input-specific. A key whose sidecar resolves to an existing file is skipped, one whose sidecar is dangling or unparsable is rewritten, zero matching files writes nothing, and two or more is `Error::AmbiguousInput { key, files }`, which fails the open since nothing can guess which file the key means. `Sweep { rules: Vec<AdoptionRule { pattern, content_type }> }` adopts by pattern instead, keying each file by its own filename **verbatim** — no `decode_key`, no extension stripping. Rules are first-match-wins; a file matching no rule is left alone. `matches_pattern` (`crawlee-storage/src/utils.rs`) is twenty lines of `*`/`?` globbing with backtracking — whole-string, case-sensitive, no path semantics, character classes or braces, and no dependency; `validate_pattern` rejects an empty pattern or one containing `/`.

Both kinds write `filename` only when it differs from `encode_key(key)`, so a plain `proxies.json` gets a sidecar byte-identical to a `set_value` one, and `size` is stated from the file. Every `Key` runs before every `Sweep`, whatever order they were declared in: a specific claim beats a blanket one.

A sweep only ever claims a file nothing else owns, which is what the `owned: HashSet<String>` built once per call is for. It holds, for every `{encoded}.__metadata__.json` in the directory listing, that `encoded` — plus, **only when `encoded` itself is absent from the listing**, the `filename` its sidecar binds. That condition is the whole reason adoption doesn't read every sidecar in the store: a record whose value file sits under its own encoded name cannot also bind elsewhere without leaving that file a stray, so in a normal store zero sidecars are read. `owned` further holds every filename any `Key` candidate *declared* — declared, not adopted, so a `Key` that skipped (already tracked) or matched nothing still shields its filenames from the sweep — and is extended with each filename a `Key` adopts, since the `listing` snapshot predates those writes. On top of `owned`, the sweep skips sidecars themselves, dotfiles (`.DS_Store`, `.gitkeep`, this crate's own `.tmp.*` write leftovers), and any file that has its own sidecar, dangling or not — a sweep never overwrites a sidecar. The listing is a `BTreeSet`, so sweep order is deterministic for test stability; there is no ambiguity to resolve, one file is one key.

Surfaced on Node `open` as an optional trailing `adopt` of `AdoptionCandidate { key?: string, files: AdoptableFile[] }` — no `key` means a sweep and the `filename`s are patterns — and on Python `open` as `(key | None, [(filename, content_type), ...])` tuples.

**KVS bare-file resolution (`resolve_value` / `resolve_existing_key`) — deprecated**: superseded by adoption above, kept until `@crawlee/fs-storage` (the only consumer) migrates. Both probe the tracked record for the literal key first (its `contentType` verbatim from the sidecar), then each caller-supplied fallback as a bare `key + extension` file: `resolve_value` takes `(extension, content_type)` pairs and, on a bare match, re-keys the result to the originally-requested key and reports the declared `content_type` (an empty string keeps the synthesized `application/octet-stream`); `resolve_existing_key` takes plain extension strings and returns the matched on-disk key for feeding to `get_public_url`. Exposed as `resolveValue`/`resolveExistingKey` (Node, with a `BareFallback { extension, contentType }` object) and `resolve_value`/`resolve_existing_key` (Python, with tuples). The bindings' `get_value`/`getValue` and `record_exists`/`recordExists` stay **strict-only** — a sidecar-less file, or a sidecar whose bound file is missing, is not a record — and the core's `require_record_metadata` bool (how the resolver reads a sidecar-less file) is not surfaced to either binding. Why it is going away: the key it resolves is invisible to `list_keys` and reads back as absent through `get_value`, and every reader has to re-declare the same probe list. The Rust methods carry `#[deprecated]`; parameters can't, so `require_record_metadata` and `bare_fallbacks` say so in their docs — which is also where the Node `@deprecated` JSDoc in `index.d.ts` comes from.

**KVS filename binding (sidecar `filename`)**: A record's sidecar is always named after its key (`{encoded_key}.__metadata__.json`), but the value file need not be — a sidecar carrying `filename` binds its key to that on-disk name instead (`INPUT` → `input.json`). Every key→file lookup resolves through it: `get_value` (and the `read_value`/`value_file_info` wrappers), `record_exists`, `delete_value`, `purge`'s keep-list, `get_public_url` (async now, since it has to read the sidecar) and the `size` backfill in `list_keys_raw`. The write paths take an optional `filename` (`set_value`/`finalize_streamed_value` in the core, `setValue`/`setValueStream` in Node, `set_value` in Python) which must pass `validate_filename` — a single path component that isn't a metadata filename — so neither a caller nor a hand-edited sidecar can point a key outside the store or shadow the store's own `__metadata__.json`. Re-binding a key deletes the file it was bound to before, so an orphan can't linger as an untracked bare file. Listing always reports keys, never bound filenames. `purge`'s keep-list spares only the file a kept key actually resolves to: it used to spare `encode_key(key)` as well, which let a stray file under that name survive every purge, unreachable through the key that had bound its way elsewhere. A value file with *no* sidecar is adoption's business (above), not this mechanism's.
...
│   ├── __metadata__.json
│   ├── {percent_encoded_key}           (value data file, unless a sidecar binds another `filename`)
...
- **KVS sidecar `filename`**: written only for a key bound to a differently-named value file (`#[serde(skip_serializing_if = "Option::is_none")]`), so ordinary sidecars stay byte-identical to what crawlee-JS and older Python clients wrote.
