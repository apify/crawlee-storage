...
**KVS adoption of sidecar-less files (`open`'s `adopt`)**: A value file can be on disk with no metadata sidecar — written into the store directory out-of-band, by a CLI or anything else that isn't this library — so no key addresses it. The canonical case is a run's input (`INPUT`, `INPUT.json`, `INPUT.bin`, ...), but nothing in the mechanism is input-specific: any key can be declared for any set of filenames. `open`/`open_with_clock` take `adopt: &[AdoptionCandidate]` (`{ key, files: Vec<AdoptableFile { filename, content_type }> }`), and right after the store `__metadata__.json` step each candidate gets the sidecar it is missing: of the declared filenames, the one that exists on disk is bound to the key (`filename` written only when it differs from `encode_key(key)`, `size` stated from the file, `content_type` taken verbatim from the declaration). Value bytes are never touched. From then on the file is an ordinary record — `get_value`, `list_keys` and `get_public_url` all see it, which is what the bare-file probe never managed. A key whose sidecar resolves to an existing file is skipped; one whose sidecar is dangling or unparseable is rewritten; zero matching files writes nothing; two or more is `Error::AmbiguousInput { key, files }` and fails the open, since nothing can guess which file the key means. Filenames go through `validate_filename` (metadata filenames are just skipped, since a sidecar is never an adoptable value). As with the old probe, the core infers nothing from an extension: which names to adopt and what type each implies is the caller's declaration. Surfaced as an optional trailing `adopt` on Node `open` (`AdoptionCandidate` objects) and Python `open` (`(key, [(filename, content_type), ...])` tuples).
