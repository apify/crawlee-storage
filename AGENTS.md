...
- **Top-level `__init__.py`/`__init__.pyi`**: `reexport_module_members!` makes pyo3-stub-gen emit a runtime `__init__.py` for the pure-Python `crawlee_storage` package (re-exports + `__all__`), gated behind `generate-init-py = ["crawlee_storage"]` in `pyproject.toml`'s `[tool.pyo3-stub-gen]`. `stub_gen` post-processes that file (`fixup_init_py`) to restore the module docstring and re-add the runtime-only `NONE_CONTENT_TYPE` re-export (added via `m.add(...)`, which the generator doesn't track), and writes its own top-level `__init__.pyi` (`write_toplevel_pyi`) re-exporting the TypedDicts for type checkers. TypedDict names are kept **out** of the runtime `__all__` (they're type-only — listing them would break `from crawlee_storage import *`) and live only in the `.pyi`.
